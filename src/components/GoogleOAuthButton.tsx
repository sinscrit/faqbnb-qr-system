...
export default function GoogleOAuthButton({
  ...
  const handleOAuthSignIn = async () => {
    ...
    try {
      ...
      console.log('🔗 OAUTH_BUTTON: INITIATING_GOOGLE_OAUTH', {
        ...
        queryParams: oauthOptions.options.queryParams,
        scopes: oauthOptions.options.scopes,
        provider: oauthOptions.provider,
        currentUrl: window.location.href,
        expectedCallbackUrl: `${window.location.origin}/auth/oauth/callback${params.toString() ? `?${params.toString()}` : ''}`,
        willPassToCallback: {
          accessCode: !!accessCode,
          email: !!email,
          code: 'will be generated by OAuth provider'
        }
      });
      ...
      console.log('🔗 OAUTH_BUTTON: OAUTH_INITIATED_SUCCESSFULLY', {
        timestamp: new Date().toISOString(),
        provider: data?.provider,
        url: data?.url ? data.url.slice(0, 150) + '...' : null,
        urlContainsAccessCode: data?.url?.includes('accessCode'),
        urlContainsEmail: data?.url?.includes('email'),
        nextStep: 'USER_WILL_AUTHENTICATE_WITH_GOOGLE_THEN_RETURN_TO_CALLBACK',
        expectedFlow: accessCode && email ? 
          'Google Auth → /auth/oauth/callback → /register?oauth_success=true' : 
          'Google Auth → /auth/oauth/callback → /login'
      });
