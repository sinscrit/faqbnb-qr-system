...
function QRPrintPageContent() {
  ...
  const handleClose = () => {
    ...
    // Navigate back to the property page in dashboard
    const parentPath = `/dashboard/properties/${propertyId}`;
    window.location.href = parentPath;
  };

  useEffect(() => {
    console.log('[QR-AUTH-DEBUG] Dashboard QR Print page mounted:', {
      propertyId,
...
export default function QRPrintPage() {
    ...
    if (hash) {
      try {
        ...
        if (decodedData.authBypass) {
          console.log('[QR-AUTH-DEBUG] Dashboard AuthGuard bypass enabled');
  ...
  console.log('[QR-AUTH-DEBUG] Dashboard QR print window - bypassing AuthGuard (backend handles auth)');
