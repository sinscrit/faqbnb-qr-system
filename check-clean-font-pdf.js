...
async function checkCleanFontPDF() {
  console.log('🔍 Checking the CLEAN-FONT-HANDLING PDF...');
  
  try {
    const pdfPath = path.join(__dirname, 'tmp', 'QR-Codes-CLEAN-FONT-HANDLING.pdf');
    ...
    while ((match = streamPattern.exec(pdfContent)) !== null) {
      ...
      try {
        ...
        const hasFullGuide = decompressedContent.includes('Full Guide');
        ...
        console.log('     Contains "Full Guide":', hasFullGuide ? '✅' : '❌');
        ...
        if (hasFullGuide) {
          console.log('   🎉 FOUND "Full Guide"! It\'s properly encoded as plain text.');
        } else if (hasTJ) {
          ...
          if (fullDecodedText.includes('Full Guide')) {
            console.log('   🎉 FOUND "Full Guide"! It was hex-encoded but decoded successfully.');
          } else {
            console.log('   ❌ "Full Guide" not found in decoded text');
          }
        } else {
        ...
        // Check positioning
        const positionMatches = decompressedContent.match(/(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s+Tm/g);
        if (positionMatches) {
          console.log('   📍 Text positioning commands found:');
          positionMatches.forEach((pos, index) => {
            console.log(`     Position ${index + 1}: ${pos}`);
          });
        }
...
checkCleanFontPDF().catch(console.error);
