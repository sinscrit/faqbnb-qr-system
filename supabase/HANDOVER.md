...
Slice 2A.1 has passed independent static validation and an isolated PostgreSQL
14 compatibility probe after correcting a real bootstrap upsert ambiguity. A
real Docker-backed Supabase 17 replay and pgTAP run remain blocked. No remote
Supabase project was linked, queried, or
mutated. Do not use `supabase link`, `db push`, `migration repair`, remote type
generation, or a project ID while the data decision is pending.

## Start Here

1. Read `../docs/restart/DATA_MIGRATION_DECISION.md`.
2. Read `../docs/restart/SLICE_2A1_DATABASE.md`.
3. Inspect `migrations/20260810000100_identity_account_foundation.sql` and
   `tests/database/2a1_identity_account.test.sql`.
4. Confirm `node_modules/.bin/supabase --version` is `2.113.0`.
...
Do not grant runtime acceptance unless the migration replays from zero and all
44 pgTAP assertions pass. `db:types` replaces
...
- This slice adds no auth UI or server application helpers.
