...
Milestone 0 and Milestone 1.1–1.3 are complete and independently validated. The email/password primary path and existing-Google compatibility boundary are closed decisions. Isolated Slices 2A.1 and 2B.1 have independent static acceptance; Docker-backed Supabase replay/RLS tests remain blocked. Slices 2A.2, 2A.3, and 2B.2 are independently validated locally, including Slice 2B.2's corrected standalone browser matrix. Isolated Slice 2C.1 has a draft-item/public-identity implementation candidate pending independent static validation and Supabase 17 replay. Real-email, staging-provider, ownership/policy, backup/restore, and accountable data-approval evidence remain active. No data disposition has been approved.
...
| 2C Item/public page | 2C.1 database candidate implemented; independent static acceptance and runtime replay pending | Create item and unauthenticated guest-safe page on staging |
...
1. Independently validate the isolated Slice 2C.1 migration, 123-assertion
   pgTAP candidate, PostgreSQL 14 compatibility evidence, and documentation.
2. When Docker becomes available, prove all three ordered migrations from zero
   plus all 44 Slice 2A.1, 77 Slice 2B.1, and 123 Slice 2C.1 real-role pgTAP/RLS
   assertions; do not apply any migration live.
3. Gather per-resource ownership/policy evidence plus backup/restore proof,
