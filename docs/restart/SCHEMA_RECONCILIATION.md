...
| Items / public ID | `items` requires unique `public_id` and `property_id`; includes name, description, QR fields, tags and source language | **Isolated 2C.1 candidate implemented; independent static/Supabase 17 acceptance pending.** It creates property-owned draft items, stable generated UUID public identity, property-scoped request idempotency, membership RLS, RPC-only creation, and a two-field published reader; QR, tags, language, media, and publication writes remain later slices | Inline type adds nonexistent live `location`; domain types make nullable timestamps/tags effectively non-null and omit source language; generated canonical types remain blocked on accepted local replay | `/dashboard2/items`, create and edit still call `/api/admin/items`; print uses `/api/user/properties/[propertyId]/items`; guest route resolves by `public_id`; the new database contract has no application consumer yet | Dashboard stats queries absent `items.location`. Create and item-list code construct `/items/{id}` while the canonical guest route is `/item/{id}`. Host APIs additionally require `properties.user_id === user.id`, conflicting with account membership semantics. The draft-only isolated candidate is not mapped to the richer live row shape | **2C.1 Item/public identity:** validate isolated draft/RLS/RPC foundation, then migrate canonical `/api/user/items`; add instructions and the only publication transition in Slice 3 before exposing the guest page |
...
4. **Slice 2C — item/public identity.** The isolated 2C.1 implementation
   candidate establishes the property FK, stable UUID public identity,
   ...
   writes. Independent static and Supabase 17 acceptance remain pending; any
