...
2. **Slice 2A.1 — identity/account.** Reproduce public profiles, accounts,
   memberships, the auth trigger and indexes. Add real two-identity RLS tests.
   Resolve one P0 account on the server; do not trust `x-current-account` or
   local storage without membership validation.
