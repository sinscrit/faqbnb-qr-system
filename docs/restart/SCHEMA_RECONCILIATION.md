...
| Properties / types | `properties(id, user_id!, property_type_id!, nickname!, address?, account_id?, timestamps)` plus seven `property_types` rows | **Isolated baseline independently statically accepted; Supabase 17 replay pending.** Slice 2B.1 reproduces the validated fields, makes `account_id` non-null only in a fresh isolated target, adds deterministic product seed values, least grants/RLS, and a server-derived first-property RPC | Inline types match the validated live columns. Domain `Property` adds nonexistent `thumbnail_url`; generated canonical types remain blocked on accepted local replay | Canonical add/edit/list still uses `/api/user/properties`; the new database contract is not yet wired into application code; item/admin compatibility APIs still join through properties | Live `account_id` is nullable while `user_id` is required. The new deterministic seed is not mapped to unknown live row values. `/api/user/properties/[propertyId]` selects nonexistent `name` and `description`. Some queries require both account and legacy user ownership, excluding valid account members | **2B.1 Property/type foundation:** fresh isolated tables/seed/RLS plus `resolve_current_property`; future preservation migration must map types, prove/backfill account ownership, preserve `user_id`, and enforce non-null only after null/orphan/ambiguous coverage is zero |
| Items / public ID | `items` requires unique `public_id` and `property_id`; includes name, description, QR fields, tags and source language | **Isolated 2C.1 independently statically accepted; 2C.2 server/API candidate pending independent acceptance; Supabase 17 runtime pending.** The database creates property-owned drafts with stable public UUIDs and an allow-listed published reader. The API revalidates cookie/account/property context for creation and uses a separate anonymous client for the exact two-field public projection; QR, tags, language, media, and publication writes remain later slices | Inline type adds nonexistent live `location`; domain types make nullable timestamps/tags effectively non-null and omit source language; generated canonical types remain blocked on accepted local replay | The new POST-only `/api/user/items` is the first canonical item consumer, but no host UI uses it yet. `/dashboard2/items`, create and edit still call `/api/admin/items`; print uses `/api/user/properties/[propertyId]/items`. The existing guest page still self-fetches the public endpoint and expects the old translated nested shape | Dashboard stats queries absent `items.location`. Create and item-list code construct `/items/{id}` while the canonical guest route is `/item/{id}`. Legacy host APIs additionally require `properties.user_id === user.id`, conflicting with account membership semantics. The draft-only isolated baseline is not mapped to the richer live row shape, and the legacy guest page is incompatible with the minimal 2C.2 DTO | **2C Item/API:** validate the server boundary, then move one host item/instruction workflow and rebuild the guest page without self-fetch; add useful instructions and the only publication transition before guest visibility |
...
- Slice 2C.2 rewrites exact `/api/public/items/[publicId]` to call only the
  allow-listed `read_public_item` RPC through a cookie-free anon-key client. It
  exposes exactly public ID/name, maps draft and unknown rows to the same 404,
  and fails malformed or multiple rows closed. It has no translation,
  analytics, demo, rate-limit, service-role, or self-fetch branch.
- The existing `/item/[publicId]` page still self-fetches that endpoint and
  expects the superseded translated nested DTO. It is not compatible with the
  2C.2 response and must be rebuilt as the guest-page slice rather than
  re-expanding the public API.
- `/api/public/items/[publicId]/languages` remains a legacy translation sibling
  using `supabaseAdmin`. It is outside the 2C.2 contract and must not be treated
  as proof of guest safety; gate or remove it when the canonical guest consumer
  migrates.
...
Slice 2C.2 now supplies that canonical creation boundary. `POST /api/user/items`
accepts one normalized name plus property/request UUID hints, freshly resolves
cookie/account/property context, and requires an exact `create_current_item`
row before returning only public ID/name. There is no list endpoint and no UI
consumer yet. The public sibling is intentionally read-only and anonymous; no
publication mutation exists.
...
3. **Slice 2B — property/type.** The independently statically accepted isolated
   2B.1 baseline seeds stable new product types, reproduces properties with
   non-null account tenancy, and adds a deterministic first-property context
   RPC. It deliberately does not claim
   that seed values match live rows and is not an additive live migration. In a
   later preservation rehearsal, map types, backfill `account_id`, preserve
   `user_id` during the compatibility window, and make account ownership
   canonical only after null/orphan/ambiguous counts are zero.
4. **Slice 2C — item/public identity.** The independently statically accepted
   isolated 2C.1 baseline establishes the property FK, stable UUID public identity,
   property-scoped retry key, draft marker, least grants/RLS, and safe creation
   and published-read RPC boundaries. It deliberately removes the phantom
   `location` contract and defers QR, tags, language, media, and publication
   writes. The 2C.2 application candidate composes those RPCs with strict
   cookie-bound creation and anonymous two-field public reads without service
   role or self-fetch. Independent acceptance remains pending for 2C.2, and
   Supabase 17 runtime replay remains pending for the database slices. Any
   preservation migration must separately reconcile the richer live item rows.
